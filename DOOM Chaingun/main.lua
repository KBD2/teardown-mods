...
function TOOL:Tick(dt)
    ...
    if InputDown("lmb") and STATE.transformTimer == 0 and not STATE.cooling then
        if not STATE.isTurret then
    ...
    if InputPressed("lmb") and not STATE.isTurret and not STATE.cooling then
        PlaySound(HANDLE_SOUND_SPINUP)
    end

    if InputReleased("lmb") and not STATE.isTurret and not STATE.cooling then
...
function TOOL:Update(dt)
    if STATE.cooling or not InputDown("lmb") then
        if STATE.state == "SPINDOWN" then
            if not STATE.isTurret then
