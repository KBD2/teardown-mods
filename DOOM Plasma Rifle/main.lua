...
function tick(dt)
    if GetString("game.player.tool") == "doomplasmagun" and GetPlayerVehicle() == 0 then
        ...
        if InputPressed("rmb") and glob.charge == 30 then
            glob.charge = 0
            for i = 1, 16 do
...
function tickFlyingProjectiles(dt)
    for i = #glob.projectiles, 1, -1 do
        ...
        if projectile.mode == "FLY" then
            ...
            particleProjectile()
            SpawnParticle(projectile.pos, Vec(0, 0, 0), 1.5 * dt)
            particleProjectileCentre()
            SpawnParticle(VecAdd(projectile.pos, randVec(-PROJECTILE_RADIUS / 5, PROJECTILE_RADIUS / 5)), Vec(0, 0, 0), 1.5 * dt)
