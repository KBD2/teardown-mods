...
function tick(dt)
	if exploding then
		if #rockets == 0 then
			...
			explode(rockets[#rockets].body, 3)
			rockets[#rockets] = nil
		end
	end
	...
	for i = #rockets, 1, -1 do
		...
		PointLight(VecSub(GetBodyTransform(rocket.body).pos, TransformToParentVec(GetBodyTransform(rocket.body), Vec(0, 0, -0.2))), 1, 0.25, 0.12, 1)
	end
...
function update(dt)
	for i = #rockets, 1, -1 do
		...
		local pos = GetBodyTransform(rocket.body).pos
		SetBodyTransform(rocket.body, Transform(pos, rocket.orientation))
		local deltaPosition = VecScale(TransformToParentVec(Transform(pos, rocket.orientation), Vec(0, 10, -1200)), 1 * dt)

		QueryRejectBody(GetToolBody())
		QueryRejectBody(rocket.body)
		hit, point, normal, shape = QueryClosestPoint(VecAdd(pos, VecScale(deltaPosition, 0.05)), 0.3)
		if hit or rocket.aliveTime > 10 then
			explode(rocket.body, 1.4)
			table.remove(rockets, i)
		else
			SetBodyVelocity(rocket.body, deltaPosition)
			createParticles(rocket.body)
		end
	end
end
...
function explode(body, size)
	local pos = GetBodyTransform(body).pos
	Delete(body)
	Explosion(pos, size)
end

function createParticles(body)
	local spawnPoint = TransformToParentPoint(GetBodyTransform(body), Vec(0.05, 0.05, -0.1))
	ParticleReset()
	ParticleType("plain")
	ParticleTile(3)
