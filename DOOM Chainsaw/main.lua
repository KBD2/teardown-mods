...
function tick(dt)
    if GetString("game.player.tool") == "doomchainsaw" and GetPlayerVehicle() == 0 then
        ...
        if InputDown("lmb") and GetBool("game.player.canusetool") and GetPlayerGrabShape() == 0 then
            glob.ddt = 0.1
        ...
        if InputDown("rmb") and GetPlayerGrabShape() == 0 then
            glob.transformTime = math.min(glob.transformTime + dt, TOOL_TRANSFORM_TIME)
        ...
        if not InputDown("lmb") and not InputDown("rmb") then
            glob.state = "READY"
        ...
        if glob.timeSinceLastRevUp > 0.5 then
            if InputPressed("lmb") and not InputPressed("rmb") then
                PlaySound(HANDLE_SOUND_REVUP, playerPos, 1)
                glob.timeSinceLastRevUp = 0
            end
            if InputReleased("lmb") and not InputPressed("rmb") then
